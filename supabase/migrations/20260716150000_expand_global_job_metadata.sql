-- ── Global Job metadata expansion ──
--
-- Adds the wider set of LinkedIn metadata fields the extension's parser can
-- now extract independently (city/country, company URL, posted-ago string,
-- applicant count, hiring insights, easy-apply/promoted flags, industry,
-- job function, benefits, sanitized description HTML) plus a distinct
-- `source_url` (the canonical job-posting page) alongside the existing `url`
-- (the apply-action URL) — these were previously conflated.
--
-- Also wires up `skills` for the first time: `upsert_global_job` now accepts
-- a `skills` array in the payload and links them via `job_skills`, which
-- existed in the schema from the start but was never populated.
--
-- Additive only — no existing column, index, or policy is changed.
...
-- Same resolution/validation rules as before; adds the new columns and
-- performs a find-or-create + link pass over `payload->'skills'`.
...
BEGIN
  ...
  ELSE
    INSERT INTO global_jobs (
      ...
      hiring_insights, easy_apply, promoted, industry, job_function, benefits
    ) VALUES (
      v_company_name,
      v_role,
      payload->>'location',
