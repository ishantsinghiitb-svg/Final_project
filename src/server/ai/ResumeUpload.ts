...
//
// Pulled out of src/server-functions/resume.ts so it has exactly ONE
// implementation shared by both callers that need it:
//   - the existing `parseResume` TanStack server function (dashboard upload)
//   - the new `/api/extension/parse-resume` route (extension upload)
// Deterministic (no AI): auth is the CALLER's job (both callers already build
// an authed `ServerSupabase` via `requireUser`) — this function just takes the
// resulting client + user. auth → load resume (RLS) → reuse-by-file-hash OR
// download+parse → persist resume_parsed + resumes.parse_status.
...
export async function parseResumeForUser(
  ...
  // already-uploaded resumes self-heal (re-parse for real) the next time this
  // runs after a parser fix/improvement, instead of forever re-serving a
  // stale (possibly incorrect) cached analysis.
  if (resume.file_hash) {
