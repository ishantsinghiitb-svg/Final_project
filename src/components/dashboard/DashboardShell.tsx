...
export function DashboardShell({ children }: { children: ReactNode }) {
  ...
  // Compact type scale (see .dash-scope in styles.css) is scoped to this
  // component's own root below, which covers everything rendered in the
  // normal tree — but Radix portals (Dialog/Select/Popover/Tooltip content)
  // mount straight onto document.body, outside that subtree, so they'd
  // silently keep the default 14px/12px text without this. Toggled on
  // mount/unmount so a marketing-page portal (there are none today, but if
  // one is ever added) is never affected.
  useEffect(() => {
    document.body.classList.add("dash-scope");
    return () => document.body.classList.remove("dash-scope");
  }, []);
  ...
  return (
    <div className="dash-scope min-h-screen bg-[oklch(0.985_0.003_250)] text-[oklch(0.2_0.02_265)]">
      <div className="grid min-h-screen lg:grid-cols-[180px_1fr]">
        <aside
          className={cn(
            "fixed inset-y-0 left-0 z-40 w-[180px] overflow-y-auto border-r border-black/5 bg-white/85 p-2 backdrop-blur transition-transform lg:sticky lg:top-0 lg:h-screen lg:translate-x-0",
        ...
        >
              ...
              {nav.map((n) => {
                ...
                return (
                  <Link
                    ...
                    className={cn(
                      "group flex items-center gap-2 rounded-lg px-2.5 py-1 text-[12px] transition-colors",
                      active
                        ? "bg-[oklch(0.95_0.02_265)] font-medium text-[#2563EB]"
                        : "text-[oklch(0.4_0.02_265)] hover:bg-black/[0.03] hover:text-[oklch(0.2_0.02_265)]",
                    )}
                  >
                    <n.icon className={cn("h-[14px] w-[14px]", active && "text-[#2563EB]")} />
          ...
          <div className="mt-3 rounded-xl border border-black/5 bg-gradient-to-br from-[#2563EB]/5 to-[#7C3AED]/10 p-3">
          ...
          <div className="mt-3 border-t border-black/5 pt-2.5">
            <Link
              ...
              className={cn(
                "flex items-center gap-2 rounded-lg px-2.5 py-1 text-[12px] transition-colors",
            ...
            >
              <HelpCircle className="h-[14px] w-[14px]" /> Help
            </Link>
            {isAdmin && (
              <Link
                to="/dashboard/admin"
              ...
              >
                <ShieldCheck className="h-[14px] w-[14px]" /> Admin
              </Link>
            )}
            <div className="mt-2 flex items-center gap-2 rounded-lg px-2 py-1">
              <UserAvatar avatarUrl={avatarUrl} initials={initials} size={28} />
              <div className="min-w-0 flex-1">
                <p className="truncate text-[12px] font-medium">{displayName}</p>
                <p className="truncate text-[10px] text-[oklch(0.5_0.02_265)]">{displayEmail}</p>
              </div>
              <button
        ...
        <div className="flex min-w-0 flex-col">
          <header className="sticky top-0 z-20 flex h-11 items-center justify-between gap-3 border-b border-black/5 bg-white/85 px-3 backdrop-blur md:px-4">
            <div className="flex items-center gap-2">
              ...
              >
                ...
                {/* truncate + nowrap: at 390px the placeholder wrapped onto a
                    ...
                    is its own content's width, not 0, so `truncate` never
                    actually got to engage — the placeholder text just kept
          ...
          <main className="min-w-0 flex-1">
            <div className="dash-container space-y-3 py-3">{children}</div>
          </main>
        </div>
      </div>
